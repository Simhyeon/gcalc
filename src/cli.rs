...
impl Cli {
    ...
    fn common_app_args(app : clap::App) -> clap::App {
        let app = app.arg(Arg::new("prob").help("Basic probability").short('p').long("probability").takes_value(true))
            ...
            .arg(Arg::new("format").help("Table format(csv|console|gfm)").short('f').long("format").takes_value(true).default_value("console"))
            .arg(Arg::new("precision").help("Precision").short('P').long("precision").takes_value(true).default_value("2"))

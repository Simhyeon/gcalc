...
pub fn get_number_as_fraction(number: f32) -> GcalcResult<f32> {
    ...
    if number >= 0.0f32 {
        ...
        if number <= 1.0f32 {
            prob = number;
        } else if number <= 100.0f32 {
            // CASE : 1.0 < num <= 100.0
            prob = number / 100.0f32;
        } else {
            return Err(GcalcError::InvalidProb(format!(
        ...
        Err(GcalcError::InvalidProb(format!(
            "Probability \"{}\" should be a positive number",
            number
        )))
    }
}
